...
_TASK_CREATE_PATTERNS = (
    r"(记一下|记一笔|帮我记|建(一个|个)?(加急|紧急)?(待办|任务)|创建任务|安排一下|提醒我|设(一个|个)?提醒|做个备忘)",
    # WO-65：无任务动词的口语句（"月底前跟进银行批复"、"下个月5号前发邮件催材料"）用时间锚定兜底
    r"(?:下个?月\s*\d+\s*号(?:前)?|月底(?:前)?|本周[一二三四五六日天](?:前|之前)?|下周[一二三四五六日天](?:前|之前)?|周[一二三四五六日天](?:前|之前)?|今天内|明天(?:上午|下午|早上|晚上)?|明早|后天(?:下午|晚上)?).{0,12}?(发邮件|发信|催材料|催流水|催件|催|跟进|联系|电话|提交|递交|交税|发合同|完成|开会|处理|准备)",
)
...
_DRAFT_EMAIL_PATTERNS = (
    r"(写|起草|拟|发|催).{0,10}?(邮件|信|催件|草稿)",
    r"(改|调整|优化|微调|重写|修改).{0,10}?(草稿|邮件|信|语气|内容)",
    r"(语气|篇幅|措辞).{0,15}?(正式|严谨|委婉|催促|精简|简短|详细)",
    r"(加上|补充|强调).{0,20}?(Finance Clause|截止日|流水|首付|贷款|说明|材料)",
)
