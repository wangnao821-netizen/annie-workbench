...
def build_system_prompt(
    ...
    lines.append("  2. 若案卷中关键要素缺失：主动向 Vera 提出 1~2 个关键缺口询问（如收入口径是 PAYG 还是自雇）；")
    lines.append("  3. 【多轮澄清直接锁定铁律】：一旦 Vera 在后续消息中给出了回答（哪怕是极短的单字或词组，如『工资』、『PAYG』、『自雇』、『无负债』等），你必须立即将该答案锁定为已确认事实！严禁再次向 Vera 重复提问相同的问题！必须直接基于该确认条件输出最终的精确方案与复议清单！")
    lines.append("  4. 【待确认模块清除规则】：当关键条件均已补齐时，末尾严禁再输出任何重复的『待确认』小节，直接给出行动建议即可！")
