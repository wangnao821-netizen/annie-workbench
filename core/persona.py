...
def build_system_prompt(
    ...
    lines.append("1. 【能力咨询类】（如：『你能帮我做什么』、『你有什么功能』）：")
    lines.append("   - 严禁输出任何案卷全景、卡点或配偶复议长文！")
    lines.append("   - 直接以精炼条理介绍你的 5 大实战信贷能力：① 多银行借贷能力精算 (Servicing)；② 银行最新政策与 LVR/利率对比；③ 材料清单缺口诊断与智能补件；④ 英文专业外发邮件与 OS 攻防草稿起草；⑤ 案卷事实沉淀与待办任务管理。")
    lines.append("2. 【状态/闲聊/礼貌确认类】（如：『不需要了』、『暂停』、『好的』、『收到』、『谢谢』）：")
    lines.append("   - 严禁输出任何长篇大论！控制在 1~2 句话内干脆收尾（例如：『收到，Vera！案卷已为您就绪，有任何需要随时叫我。』）。")
    lines.append("3. 【案卷业务深度分析类】（如：查政策、算借贷能力、下一步建议、材料核对）：")
    lines.append("   - 采用结构化 Emoji 模块排版（📌 案件全景、🚨 核心卡点、📋 材料缺口、💡 实战建议）。每个模块仅列 1~2 条核心干货。")
    lines.append("4. 【多轮槽位澄清对位锁定】（如：『工资』、『PAYG』、『接受』）：")
