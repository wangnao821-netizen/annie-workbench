"""P3 结构化槽位落库与直读验证脚本 — 需显式执行，不参与 pytest 收集。

用法: python tests/test_slots_persistence.py
"""
...
from core.facts.slots import (
    build_confirmed_slots_prompt_block,
    get_case_slots,
    set_slot_fact,
)
...
def main() -> None:
    ...
    set_slot_fact(case_id, "applicant.spouse_income", "1000000", db, category="applicant")
    set_slot_fact(case_id, "applicant.spouse_income_type", "PAYG", db, category="applicant")
    set_slot_fact(case_id, "applicant.co_borrower_accepted", "客户明确接受加配偶共同借款", db, category="applicant")
