...
关于贵行针对 PERSON_1 案件提出的 OS 条件 (rental income流水)，已附上 Accountant Letter 说明及 12 个月 BAS 补充材料，请协助优先复核，以便在 Finance Due 前完成条件解下。`;

const DEFAULT_EN_DRAFT = `Dear ANZ Credit Team,

Re: OS Conditions for Application PERSON_1.
  ...
  return (
    <div className="w-full xl:w-[400px] flex-shrink-0 flex flex-col space-y-4" id="os-draft-column">
      <div className="flex items-center justify-between pb-2 border-b" style={{ borderColor: 'var(--border)' }}>
        ...
        <span className="text-[10px] font-mono px-2 py-0.5 rounded font-bold bg-emerald-500/10 text-emerald-500">
          编辑中
        </span>
      </div>
      ...
      <div 
        ...
        style={{ backgroundColor: 'var(--amber-soft, rgba(245, 158, 11, 0.1))', borderColor: 'var(--amber, #f59e0b)' }}
      ...
      <div className="flex-1 space-y-3 overflow-y-auto no-scrollbar">
        ...
        <div className="space-y-1">
          <label className="text-[11px] font-bold text-muted flex items-center justify-between">
            <span>【中文版草稿】</span>
            <span className="font-mono text-[10px]">{cnDraft.length} 字</span>
          </label>
          <textarea
            value={cnDraft}
        ...
        <div className="space-y-1">
          <label className="text-[11px] font-bold text-muted flex items-center justify-between">
            <span>【英文版草稿】</span>
            <span className="font-mono text-[10px]">{enDraft.length} words</span>
          </label>
          <textarea
            value={enDraft}
      ...
      <div className="pt-2 border-t flex items-center space-x-2" style={{ borderColor: 'var(--border)' }}>
        <motion.button
          ...
          className="flex-1 py-2.5 rounded-xl text-xs font-semibold flex items-center justify-center space-x-1.5 text-white cursor-pointer shadow-xs"
