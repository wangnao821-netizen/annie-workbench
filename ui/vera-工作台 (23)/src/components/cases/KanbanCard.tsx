...
export function KanbanCard({ caseData, onClick, onPointerDown, isDragging }: KanbanCardProps) {
  ...
  return (
    <motion.div
      ...
      } ${isOsCategory || osPendingCount > 0 ? 'border-l-4 border-l-rose-500' : ''}`}
      style={{
    ...
    >
      ...
      <div className="flex items-start justify-between min-w-0">
        <div className="min-w-0 flex-1 pr-1">
          <div className="flex items-center space-x-1.5 flex-wrap gap-y-1">
            ...
            {/* OS 角标 */}
            {osPendingCount > 0 && (
              <span id="case-os-badge" className="flex items-center text-[10px] font-bold text-white bg-rose-500 px-1.5 py-0.2 rounded flex-shrink-0">
            ...
            {/* 关联案件 */}
            {linkedCount > 1 && (
              <span id="case-linked-badge" className="px-1.5 py-0.2 rounded text-[10px] font-semibold bg-blue-500/10 text-blue-600 flex-shrink-0">
                ×{linkedCount} 关联
              </span>
            )}
          </div>
      ...
      {/* 一句话摘要 */}
      {summary && (
        <p className="text-[10px] text-muted truncate leading-tight" style={{ color: 'var(--text-secondary)' }}>
          {summary}
        </p>
      )}
        ...
        <div className="w-full h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: 'var(--bg-app)' }}>
          <div
            ...
            style={{
              ...
              backgroundColor: isOsCategory || osPendingCount > 0 ? 'var(--amber, #f59e0b)' : 'var(--accent)',
            }}
          />
        </div>
      </div>
      ...
      <div className="flex items-center justify-between text-[10px] font-mono text-muted pt-1 border-t" style={{ borderColor: 'var(--border)' }}>
        ...
        {deadlineDays !== null ? (
          <span
            id="case-due-badge"
            className={`px-1.5 py-0.2 rounded text-[10px] font-bold flex items-center space-x-0.5 ${
              deadlineDays < 3
          ...
          <span className="font-bold text-amber-500 flex-shrink-0">{deadline || '跟进中'}</span>
        )}
