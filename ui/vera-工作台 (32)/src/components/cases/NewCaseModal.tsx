...
export function NewCaseModal({ open, onClose, onCreated }: NewCaseModalProps) {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" id="new-case-modal-overlay">
        <motion.div
          initial={{ opacity: 0, scale: 0.95, y: 8 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.95, y: 8 }}
          transition={{ type: 'spring', damping: 28, stiffness: 350 }}
          className="w-full max-w-xl rounded-2xl border shadow-2xl flex flex-col overflow-hidden glass-card"
