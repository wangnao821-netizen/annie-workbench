        ...
        {activeCaseInfo ? (
          <div className="flex items-center space-x-2 truncate">
            ...
            <span className="text-[11px] text-muted hidden md:inline ml-2 flex items-center space-x-1">
              <Brain className="w-3.5 h-3.5 text-purple-500 inline mr-1" />已注入案件上下文
            </span>
          </div>
        ) : (
      ...
      <div className="p-4 space-y-3 flex-1 overflow-y-auto no-scrollbar">
        {!activeCaseInfo && messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center p-6 text-center space-y-4">
            <div className="p-4 rounded-2xl bg-purple-500/10 text-purple-500"><Sparkles className="w-8 h-8" /></div>
            ...
            <motion.button whileTap={{ scale: 0.95 }} onClick={() => setNewCaseOpen(true)} id="global-chat-new-case-btn"
              className="px-4 py-2 rounded-xl text-xs font-bold text-white flex items-center space-x-1.5 cursor-pointer shadow-xs" style={{ backgroundColor: 'var(--accent)' }}>
