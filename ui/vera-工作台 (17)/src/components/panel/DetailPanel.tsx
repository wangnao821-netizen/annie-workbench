...
export function DetailPanel() {
  ...
  return (
    ...
    >
      ...
      <div className={`flex-1 flex overflow-hidden ${isWide ? 'flex-row' : 'flex-col'}`}>
        ...
        {isWide && chatCollapsed && (
          <div className="w-10 flex-shrink-0 border-l flex flex-col items-center py-2" style={{ backgroundColor: 'var(--bg-panel)', borderColor: 'var(--border)' }}>
            <button onClick={() => setChatCollapsed(false)} title="展开 AI 对话" id="chat-expand-btn" className="p-1.5 rounded-lg hover:bg-black/5 dark:hover:bg-white/10 cursor-pointer text-xs font-bold">🤖</button>
          </div>
        )}
      ...
      <ContextDrawer title="案件大脑" open={drawer === "brain"} onClose={() => setDrawer(null)}>
