...
export function OverviewTools({ context }: OverviewToolsProps) {
  ...
  return (
    <div className="p-3.5 rounded-2xl border space-y-2.5" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}>
      ...
      {expanded && (
        <div id="overview-ai-context-preview" className="p-3 rounded-xl border bg-black/5 dark:bg-white/5 space-y-2 text-xs" style={{ borderColor: 'var(--border)' }}>
          <div className="flex items-center justify-between text-[11px] font-medium text-muted">
            <span>预览即时生成的全景 Prompt 摘要:</span>
            <button
              type="button"
              onClick={handleCopy}
              className="px-2 py-0.5 rounded border bg-purple-500/10 text-purple-600 flex items-center space-x-1 cursor-pointer font-bold hover:opacity-80"
            >
              {copied ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
              <span>{copied ? '已复制' : '复制全文'}</span>
            </button>
          </div>
          <pre className="text-[11px] font-mono leading-relaxed whitespace-pre-wrap p-2 rounded bg-black/10 dark:bg-black/30 max-h-48 overflow-y-auto" style={{ color: 'var(--text-primary)' }}>
            {contextText}
          </pre>
        </div>
      )}
      ...
      <div className="flex items-center justify-between text-[10px] text-muted pt-1">
        ...
        <span className="font-mono text-[9px]">// 统一端点: GET /api/cases/&#123;id&#125;/context</span>
      </div>
    </div>
  );
}
