...
> F-29~F-38 已交付（(52)-(56)，Codex 审查通过）；**F-39 已出（2026-08-15，中栏体验细节）**。
> 日期：2026-08-14
...
## 十五、2026-08-15 增补：中栏体验细节（F-39 定稿）

> Vera 对 (56) 中栏提出四件事，Codex 分析后出 F-39 批次（纯前端，接 WO-44 端点）：

1. **底部「已记录 N 条」挂栏** → 收进输入框旁小胶囊（点击开记录抽屉）；有 pending 待确认时加红点防漏确认；
2. **快捷发问 chips** → 输入框上方一行横向滚动（复用 QUICK_ASKS 8 项）；
3. **顶部数量徽章** → 「任务」显示未完成数（逾期红）、「清单」显示未收完数（status 非 received/confirmed）；
4. **发文件/图片识别入口** → 输入框 📎 附件按钮（仅案件模式）→ importCaseFile（复制进案件文件夹）→
   previewCaseFile（OCR text_preview）→ 识别文本以「📄 文件识别」消息进对话；
5. **OCR 能力定案（WO-45）**：复用老项目已迁入的 LiteParse 链路（core/pipeline/parser.py），
   不引入 RapidOCR/Docling；补兜底依赖 + 合成样本实测，效果不足再评估补充引擎。
