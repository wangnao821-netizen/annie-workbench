...
## 十六、2026-08-16 增补：中栏快捷折叠 + 邮件悬浮窗 + 文件原文预览（F-40 / WO-46 定稿）

> Vera 对 (57) 提出三问题，方案已拍板：

1. **文件预览默认出原文**：后端 raw 只读文件流端点（WO-46，PathGuard 同案件/≤20MB/白名单），
   前端预览双 tab——默认「原文」（PDF/图片内嵌），「解析内容」保留 OCR 文本（F-40）；
2. **中栏快捷按钮折叠**：取消 chips 行，输入框左侧固定「📌已记录｜📎附件｜⚡工具」；
   ⚡ 工具菜单 = 工具动作（计算器/写邮件/建案件/去文件夹）+ 快捷提问（缺口预判/申报检查/到期逾期/查政策）；
   全局咨询只显示全局可用项；
3. **邮件独立悬浮窗 MailComposeModal**：⚡ 菜单「写补件邮件」打开（仅案件模式）；
   收件人/主题/英文正文/披露徽章/版本；操作 = 保存草稿（POST /api/drafts 进草稿箱）+ 复制英文；
   **无发送按钮（红线：只出草稿）**。

## 十七、2026-08-16 定稿：三共创弹窗深谈对齐（方案 A）

> 举一反三核对（主文档 §二 vs 实现）发现：共创类（邮件/催件/OS 回复）定稿为「弹窗深谈（独立子会话）」，
> 但 WO-27/F-15 实际实现为主对话 FlowDialogCard（draft 写主对话表、卡片在消息流）。Vera 拍板方案 A 对齐定稿：

1. **呈现**：三共创流程统一走 CoCreateDialog 独立弹窗深谈（F-40 v2 + WO-46b）；主对话只保留触发/恢复入口；
2. **拉案件全景**：draft_email 现不注入案件全景（LLM 不知客户/银行/阶段/补件要求）——WO-46b 补全景注入；
3. **澄清意图 1-3 轮**：流程包现一步出 V1——WO-46b 补 clarify 环节；
4. **收尾**：confirm 现只写事件——WO-46b 补可选建待办（create_todo）；
5. **DraftCard 出口**：「复制/翻译」现为占位 toast——F-40 v2 修复（真复制 + 中文对照）；
6. **可中断恢复**：session_id（draft: 前缀）持久，弹窗关闭后经「继续共创」恢复；
7. **后台自主类**：跟进提醒缺独立 job——WO-47 补齐（默认关闭）。
