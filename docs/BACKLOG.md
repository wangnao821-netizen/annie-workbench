...
- **界面职责重排 + 任务/清单双 Agent（2026-08-14 定稿，定稿文档 docs/CASE大脑_客户上下文维护与任务视图_定稿.md）**：
  ...
  - ✅ **WO-42 已完成**（2026-08-14，14110e2：BrainFact 锁定/修正/披露标记 + 蒸馏锁定保护；迁移 b8c7d6e5f4a3 head；11 专项、全量 1043）→ 📋 **F-30 提示词已出待前端**（客户全景页 = 上下文维护中心 + 时间线）
  - 🐍 **F-31 规划待出**（待办工作台退役：列表并入首页/中栏；8 类详情组件抽 TaskDetailOverlay；OsWorkbench 保留 OS 专用；清单 tab 退役复用中栏抽屉）
...
| 项 | 出处 | 状态 | 下一步 |
|----|------|------|--------|
| **人格设定（AI 拟人化：名字/语气）** | 主文档 §八#1、能力矩阵 §六、Phase1 构想（名字+固定语气：专业/直接/不废话/会提醒/中文/记忆人格）、架构分析 v2（阶段 0 给 3 个人格候选让 Vera 选） | ✅ 已实施（2026-08-14）：四种内置 + 默认 A + 运行期名字/称呼/人格切换 | config/persona.yaml + core/persona.py + 接入角色层 + /api/settings/assistant（后端完成）；📋 F-32 前端（首次引导卡 + 设置页 AI 助手卡片） |
