...
> 最后更新：2026-08-14（E 文档收口：WO-35~39b / F-22~26b 状态归拢）
...
- 文件操作：Vera 主动要求才执行（放入/改名/移动），PathGuard 校验，绝不自主（✅ **WO-44 已完成** 2026-08-14，f4d8049：6 端点 + agent-file + file_ops 流程包，13 专项、全量 1060；F-34 前端已在 (54)，待联调；两处偏离已登记——pai 工具计数 10→11、file_ops.yaml 触发语去裸词"文件"防吞 WO-32 路由）
...
- ✅ 会话压缩 → **WO-35 已完成**（2026-08-14，commit 8dc1de6：窗口外消息蒸馏为 CaseContextEvent 摘要 + 对话层前置注入；专项 14、全量 923；施工单/提示词 docs/flash_specs/wo-35-*）
- ✅ 能力盘点矩阵 → **已同步**（2026-08-14，commit 977108e：前端条目 (49) 全量核对 + WO-35/37/38/39/39b + 中澳协同节）
...
- 前端 F-22 已完成（2026-08-14，(45) 验收通过）：能力中心对话触发状态同步（available+enabled"对话可触发"、顶部触发语动态化）
- 前端 F-23 已完成（2026-08-14，(49) 验收通过）：顶栏精简（Logo 去副标题+去"今日工作台"Tab+头像下拉收纳低频页）+ 更多功能重组（导入历史/数据迁移从导航移除）；窗口控制按钮明确不做（Electron WO-05 系统原生标题栏）
- 导入历史定位澄清（2026-08-14）：新建案件"历史导入"模式 = 历史/结案客户轻量建壳（is_imported=true 不触发提醒 + 一段话 AI 预填），**V1 保留**；导入历史页 = 迁移工具台账，移设置页"数据与备份"区（Phase 2）；重型批量导入（文件夹 OCR）暂停待 V2
- ✅ 澳洲时区/假期 → **WO-39 已完成**（2026-08-14，commit 816fa5f：GET /api/holidays + 案件截止日休息日预警；opencode 交付，12 专项、全量 962）+ **WO-39b 中国长假已完成**（c7ab440，china/next_china；专项 15、全量 965）
- ✅ 前端 F-24~F-26b 已完成（2026-08-14，(49) 验收通过）：顶栏堪培拉时间 + 面板（三时区/三州工作日/办公重叠/夏令时/日历三色/假期 4 条/倒计时/A 下一个工作日/B 今日摘要/C 中国长假/中国红色日历标记/hover 假期名）；F-25 宽度回归 320px + 不透明背景
...
- ✅ 决策先例检索 → **WO-37 已完成**（2026-08-14，commit 6af911c：同客户+同类场景先例进 case_chat 上下文；全量 950）
...
- **界面职责重排 + 任务/清单双 Agent（2026-08-14 定稿，定稿文档 docs/CASE大脑_客户上下文维护与任务视图_定稿.md）**：
  - ✅ **WO-41 已完成**（2026-08-14，39347e3：create_task 扩 deadline/priority/assignee + chat 工具 + agent-task + task_ops 流程包；V1 只做建任务；10 专项、全量 1013）
  - ✅ **WO-43 已完成**（2026-08-14，6bd2538：checklist_library_custom 总项库 + 新增端点 + master_picker 合并 + query/preview chat 工具 + AI 按需重选；16 专项、全量 1013；迁移 c3f9e7a2b1d4 head）
  - 📋 **F-29 提示词已出待前端**（中栏双悬浮：任务抽屉 + 清单抽屉（业务分类/新增沉淀）+ 右栏重组：关键截止块/待办排序红黄绿+分类徽标/下部折叠/只读硬规则）
  - ✅ **WO-42 已完成**（2026-08-14，14110e2：BrainFact 锁定/修正/披露标记 + 蒸馏锁定保护；迁移 b8c7d6e5f4a3 head；11 专项、全量 1043）→ ✅ **F-30 已交付**（(53)）⚠️ 实现与定稿颠倒（维护中心误入右栏），**F-36/F-37 对齐修正中**
  - ✅ **F-35 / F-36 / F-37 已交付**（(55)，Codex 审查）：居中面板 / 右栏只读态势 / 全景页维护中心已落地；三处问题待 F-38 补丁（右栏披露标记、逾期提醒开抽屉、全景页空态文案）
  - 📋 **F-38 补丁已出**（右栏披露标记 + 中栏逾期提醒联动 + 全景页文案）；**F-38 补丁二**追加（右栏关键截止/下一步/风险空态占位，区块骨架常驻）
  - 📋 **F-38 补丁三**追加（事实快照折叠标题加 🔒 角标，不展开也可见"有不披露信息"）
  - 📋 **F-31 提示词已出待前端**（待办工作台退役：列表并入首页/中栏；8 类详情组件抽 TaskDetailOverlay；OsWorkbench 保留 OS 专用；清单 tab 退役复用中栏抽屉）
...
| 项 | 出处 | 状态 | 下一步 |
|----|------|------|--------|
| **人格设定（AI 拟人化：名字/语气）** | 主文档 §八#1、能力矩阵 §六、Phase1 构想（名字+固定语气：专业/直接/不废话/会提醒/中文/记忆人格）、架构分析 v2（阶段 0 给 3 个人格候选让 Vera 选） | ✅ 已实施（2026-08-14）：四种内置 + 默认 A + 运行期名字/称呼/人格切换 | config/persona.yaml + core/persona.py + 接入角色层 + /api/settings/assistant（后端完成）；✅ F-32 已交付（(53)，Codex 验收通过，待真机） |
| **主动询问节奏**（一次问几个、什么时候问） | 主文档 §八#2、缺口清单 #3（V1 范围已收口，节奏细节未定） | ✅ 已并入 persona common_rules（2026-08-14） | 一次最多 1-2 个问题，关键缺口/截止日临近时才问 |
| **防串案协议② 事实归属校验** | 缺口清单 #2 三层防线（定稿 2026-08-12） | ✅ 已实施（2026-08-14，5a2ed97）：record_fact 归属校验，其他客户名 → attribution_suggest 卡，未确认不写入/不进蒸馏；4 专项、全量 1047 | 📋 F-33 前端卡展示（切换/仍记录/取消）+ 递交模式二次确认 |
| **回复带名规则** | 缺口清单 #2（定稿：涉及事实确认/外线草稿/记录动作时以"张三（CBA）"开头） | ✅ 已并入 persona common_rules（2026-08-14） | 已随人格注入生效 |
| 记忆人格主动召回（"上次你说想换 CBA，定了吗？"） | Phase1 构想 L79 | 🅿️ V2（依赖主动询问/承诺机制） | V2 主动询问时评估 |
| 快速粘贴记录本入口 | 缺口清单 #1 倾向（"对话为主 + 记录本保留快速粘贴"） | ⚠️ 未做专门入口 | 可选低优；对话"记一笔"已覆盖大部分场景，暂不立项 |

已核实无需担心：外线草稿"收件客户确认" ✅、统一建案页一句话解析/文件导入预填 ✅、双线 track 语义 ✅、Screenpipe V2 ✅。
