...
- 前端 F-23 已入批次（2026-08-14，方案已拍板）：顶栏精简（Logo 去副标题+去"今日工作台"Tab+头像下拉收纳低频页）+ 更多功能重组（导入历史/数据迁移从导航移除）；窗口控制按钮明确不做（Electron WO-05 系统原生标题栏）
...
- 澳洲时区/假期（2026-08-14 拍板 V1 全量）：总部堪培拉（ACT）+ 客户集中悉尼（NSW）/布里斯班（QLD，无夏令时）；V1 含 三时区对照/三州工作日/办公重叠/夏令时/本月日历/假期列表/倒计时/**案件截止日休息日预警**；**WO-39 已起草**（docs/flash_specs/wo-39-au-holidays.md）+ **F-24 已入批次**（顶栏时间 + 面板）
- 前端 F-25 已入批次（2026-08-14）：AU 时间面板加宽 w-[420px] + 不透明背景 + 一屏无滚动 + 布里斯班标签单行（Vera 验收反馈两项均验证属实）
- 前端 F-26 已入批次（2026-08-14，方案拍板）：AU 面板宽度回 320px + 假期单列固定 4 条 + 日历压缩 + A 下一个工作日 / B 今日业务摘要 / C 中国长假倒计时（C 后端 **WO-39b 已完成** c7ab440：GET /api/holidays 扩展 china/next_china）
- 前端 F-26b 已入批次（2026-08-14，Vera 反馈）：中国长假首日红色标记进日历 + 图例"中国（红）"（F-26 主体已验收，(48)）
