...
export function DraftEditor({ actionId }: DraftEditorProps) {
  ...
  if (error) {
    return (
      <div className="p-4 rounded-2xl border border-[var(--red-soft)] bg-[var(--red-soft)] text-[var(--red)] space-y-3">
    ...
    <div className="rounded-2xl p-5 border space-y-4 shadow-2xs" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }} id="draft-editor">
      ...
      <div className="flex items-center justify-between pb-2 border-b" style={{ borderColor: 'var(--border)' }}>
        ...
        <span
          className={`px-2 py-0.5 rounded text-xs font-semibold ${
            draft.status === 'confirmed' ? 'bg-[var(--green-soft)] text-[var(--green)]' : 'bg-[var(--purple-soft)] text-[var(--purple)]'
          }`}
        >
      ...
      <div className="space-y-2 text-xs">
        ...
        <div className="p-3.5 rounded-xl border text-xs leading-relaxed font-mono whitespace-pre-wrap space-y-3" style={{ backgroundColor: 'var(--bg-app)', borderColor: 'var(--border)', color: 'var(--text-primary)' }}>
          <div>
            <div className="text-[11px] font-sans font-bold text-[var(--accent)] pb-1">=== 中文 (Chinese) ===</div>
            <p>{draft.body_zh}</p>
          </div>
          <div>
            <div className="text-[11px] font-sans font-bold text-[var(--purple)] pb-1">=== English ===</div>
            <p>{draft.body_en}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between pt-2 border-t" style={{ borderColor: 'var(--border)' }}>
        <motion.button
          ...
          className="px-4 py-2 rounded-xl text-xs font-semibold flex items-center space-x-1.5 cursor-pointer shadow-xs"
        ...
        {versions.length > 0 && (
          <div className="flex items-center space-x-1.5 overflow-x-auto no-scrollbar">
            ...
            <span className="text-[11px]" style={{ color: 'var(--text-muted)' }}>版本:</span>
            {versions.map((v) => (
              <button
                key={v.version}
                id={`draft-version-${v.version}`}
                onClick={() => rollbackDraft(actionId, v.version)}
                className={`px-2 py-0.5 rounded-md text-xs font-mono cursor-pointer border transition-colors ${
                  v.version === draft.version ? 'font-bold border-[var(--purple)] bg-[var(--purple-soft)] text-[var(--purple)]' : 'hover:opacity-80'
                }`}
