...
import { CheckCircle2, Clock, FileCheck, FileText, RefreshCw } from 'lucide-react';
...
import { getCaseFiles } from '../../../services/api/cases';
import { CaseFileResponse } from '../../../types/api';
...
export function FileMatchDetail({ task }: FileMatchDetailProps) {
  ...
  const fetchFiles = useCallback(async () => {
    const targetCaseId = task.caseId || 'CASE_001';
    setLoadingFiles(true);
    try {
      const files: CaseFileResponse[] = await getCaseFiles(targetCaseId);
      if (files && files.length > 0) {
        const mapped: FileMatchResult[] = files.map((f) => {
          let st: "matched" | "unmatched" | "discrepancy" = "matched";
          if (f.status === "unmatched") st = "unmatched";
          else if (f.status === "discrepancy") st = "discrepancy";

          const conf = f.confidence !== undefined && f.confidence !== null
          ...
          return {
            ...
            status: st,
            targetChecklistLabel: f.assigned_type || "待识别材料",
            extractedInfo: `${f.original_name} · 分类: ${f.assigned_type} · 置信度: ${conf ? conf + '%' : '--'}`,
            aiSuggestion: f.assigned_type,
            aiConfidence: conf,
          };
        });
      ...
      // Fallback to initial default if API has no files
    } finally {
    ...
    <div className="space-y-6" id="file-match-detail-view">
        ...
        <div className="flex items-center justify-between pb-2 border-b" style={{ borderColor: 'var(--border)' }}>
          ...
          <div className="flex items-center space-x-2">
            <motion.button
              ...
              className="p-1 rounded text-xs text-muted hover:text-primary cursor-pointer disabled:opacity-50"
              title="刷新文件列表"
            >
            ...
            <span className="text-[11px] font-mono px-2 py-0.5 rounded font-semibold" style={{ backgroundColor: 'var(--green-soft)', color: 'var(--green)' }}>
              自动匹配成功 {matchResults.filter((m) => m.status === 'matched').length} 项
            </span>
          </div>
        </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-[11px] font-mono text-muted">({MOCK_EXTRACTED_FILES.length} 份关键文件)</span>
        </div>
