      ...
      <div className="flex items-start justify-between min-w-0">
        <div className="min-w-0 pr-2">
          <div className="flex items-center space-x-1.5 flex-wrap gap-y-1">
            <h3 className="text-sm font-bold group-hover:text-[var(--yellow)] transition-colors truncate" style={{ color: 'var(--text-primary)' }}>
              {clientName}
            </h3>

            {/* OS 角标 */}
            {osPendingCount > 0 && (
              <span id="case-os-badge" className="px-1.5 py-0.2 rounded text-xs font-bold bg-[var(--red)] text-white flex items-center space-x-0.5">
            ...
            {/* 关联案件 */}
            {linkedCount > 1 && (
              <span id="case-linked-badge" className="px-1.5 py-0.2 rounded text-xs font-semibold bg-[var(--accent-soft)] text-[var(--accent)]">
                ×{linkedCount} 关联
              </span>
            )}
          </div>
        ...
        <span className="text-[11px] font-mono font-bold px-2 py-0.5 rounded-lg border flex-shrink-0" style={{ backgroundColor: 'var(--bg-app)', borderColor: 'var(--border)', color: 'var(--accent)' }}>
          {stage}
        </span>
      </div>
      ...
      <div className="pt-2 border-t flex items-center justify-between text-[11px]" style={{ borderColor: 'var(--border)', color: 'var(--text-muted)' }}>
        <div className="flex items-center space-x-1 font-mono">
          {deadlineDays !== null ? (
            <span
              id="case-due-badge"
              className={`px-1.5 py-0.5 rounded text-xs font-bold flex items-center space-x-1 ${
                deadlineDays < 3
