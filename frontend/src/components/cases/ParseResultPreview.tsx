...
export function ParseResultPreview({ isParsing, hasParsed, highlightedFields }: ParseResultPreviewProps) {
  ...
  if (isParsing) {
    return (
      <div id="parse-result-preview" className="p-3 rounded-xl border bg-[var(--purple-soft)] flex items-center space-x-2 text-xs text-[var(--purple)] animate-pulse" style={{ borderColor: 'rgba(168,85,247,0.2)' }}>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-1.5 font-bold text-[var(--purple)]">
          <Sparkles className="w-4 h-4" />
          <span>AI 解析预填完成</span>
        </div>
        <span className="text-[11px] px-2 py-0.5 rounded-md font-medium text-[var(--green)] bg-[var(--green-soft)] flex items-center gap-1">
      ...
      {highlightedFields.length > 0 && (
        <div className="pt-2 border-t space-y-1.5" style={{ borderColor: 'var(--border)' }}>
          <div className="flex items-center space-x-1 text-[11px] font-bold text-[var(--yellow)]">
          ...
          <div className="flex flex-wrap gap-1.5 pt-0.5">
            {highlightedFields.map((fieldKey) => (
              <span
                key={fieldKey}
                className="px-2 py-0.5 rounded-md text-xs font-semibold bg-[var(--yellow-soft)] text-[var(--yellow)] border border-[var(--yellow-soft)]"
              >
      ...
      <p className="text-[11px] font-mono italic" style={{ color: 'var(--text-muted)' }}>
