...
export function OverviewFacts({ context }: OverviewFactsProps) {
  ...
  const assessorName = currentCase?.assessorName;
  const lenderRef = currentCase?.lenderRef;
  const activeBlocker = currentCase?.activeBlocker;

  return (
    <div className="space-y-3" id="overview-facts-wrapper">
      {/* Active Blocker Alert Banner if exists */}
      {activeBlocker && (
        <div 
          className="p-3.5 rounded-2xl border border-red-500/30 bg-red-500/10 text-red-400 flex items-center justify-between shadow-xs"
          id="overview-active-blocker-banner"
        >
          <div className="flex items-center space-x-2.5 min-w-0">
            <AlertOctagon className="w-4 h-4 text-red-500 flex-shrink-0 animate-pulse" />
            <span className="font-bold text-xs text-red-500 flex-shrink-0">⚠️ 案件暂停/阻断中：</span>
            <span className="text-xs font-medium text-red-300 truncate">{activeBlocker}</span>
          </div>
          <span className="text-[11px] px-2 py-0.5 rounded bg-red-500/20 text-red-300 font-bold flex-shrink-0 ml-2">
            需优先处理
          </span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3" id="overview-facts-grid">
        ...
        <div className="p-3.5 rounded-2xl border space-y-1.5" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}>
          <div className="flex items-center justify-between text-[11px] font-bold" style={{ color: 'var(--text-muted)' }}>
            <span className="flex items-center space-x-1"><UserCheck className="w-3.5 h-3.5 text-[var(--accent)]" /><span>身份与关系</span></span>
            <span className="px-1.5 py-0.5 rounded text-xs bg-[var(--accent-soft)] text-[var(--accent)]">关联 {linkedCasesCount} 个案件</span>
          </div>
          <div className="text-xs space-y-0.5">
            ...
            {assessorName && (
              <p className="text-[11px] text-[var(--purple)] font-semibold flex items-center space-x-1 pt-0.5">
                <UserCheck className="w-3 h-3" />
                <span>信贷审批官: {assessorName}</span>
              </p>
            )}
            {facts.special_circumstances && (
              <p className="text-[11px] text-[var(--yellow)] font-medium">特殊情况: {facts.special_circumstances}</p>
            )}
          </div>
        </div>
        ...
        <div className="p-3.5 rounded-2xl border space-y-1.5" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}>
          <div className="flex items-center justify-between text-[11px] font-bold" style={{ color: 'var(--text-muted)' }}>
            <span className="flex items-center space-x-1"><Landmark className="w-3.5 h-3.5 text-[var(--accent)]" /><span>交易结构</span></span>
            <span className="font-mono text-[var(--accent)] font-bold">{facts.lender || '未设定'}</span>
          </div>
        ...
        <div className="p-3.5 rounded-2xl border space-y-1.5" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}>
          <div className="flex items-center justify-between text-[11px] font-bold" style={{ color: 'var(--text-muted)' }}>
            <span className="flex items-center space-x-1"><Clock className="w-3.5 h-3.5 text-[var(--yellow)]" /><span>关键日期倒计时</span></span>
            {daysLeft !== null ? (
              <span className={`px-1.5 py-0.5 rounded text-xs font-bold ${daysLeft < 3 ? 'bg-[var(--red-soft)] text-[var(--red)]' : daysLeft < 7 ? 'bg-[var(--yellow-soft)] text-[var(--yellow)]' : 'bg-[var(--green-soft)] text-[var(--green)]'}`}>
                Finance Clause: {daysLeft <= 0 ? '已到期' : `${daysLeft} 天`}
              </span>
            ) : (
              <span className="text-[11px] text-muted">未设置</span>
            )}
          </div>
        ...
        <div className="p-3.5 rounded-2xl border space-y-2" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}>
          <div className="flex items-center justify-between text-[11px] font-bold">
            <span className="flex items-center space-x-1" style={{ color: 'var(--text-primary)' }}><CheckCircle2 className="w-3.5 h-3.5 text-[var(--green)]" /><span>清单进度 ({done}/{total})</span></span>
            <span className="font-mono font-bold text-[var(--green)]">{pct}%</span>
          </div>
          ...
          <div id="overview-checklist-gaps" className="space-y-1">
            {checklist.missing.slice(0, 5).map((item, idx) => (
              <div key={idx} className="p-1.5 rounded bg-[var(--bg-app)] border text-[11px] flex items-center justify-between" style={{ borderColor: 'var(--border)' }}>
                <span className="truncate max-w-[200px]" style={{ color: 'var(--text-primary)' }}>{item}</span>
                <span className="px-1 text-[11px] font-bold text-[var(--yellow)] bg-[var(--yellow-soft)] rounded">缺失</span>
              </div>
            ))}
            {checklist.missing.length === 0 && (
              <div className="text-[11px] text-[var(--green)] font-medium pt-1">全量清单材料均已到位</div>
            )}
          </div>
        </div>
        ...
        <div className="p-3.5 rounded-2xl border space-y-1.5" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}>
          <div className="flex items-center justify-between text-[11px] font-bold">
            <span className="flex items-center space-x-1" style={{ color: 'var(--text-primary)' }}>
              <AlertTriangle className="w-3.5 h-3.5 text-[var(--yellow)]" />
              <span>银行 OS 条件</span>
            </span>
            {os.pending_count > 0 ? (
              <span className="px-1.5 py-0.5 text-xs font-bold rounded bg-[var(--red-soft)] text-[var(--red)]">{os.pending_count} 项待处理</span>
            ) : (
              <span className="px-1.5 py-0.5 text-xs font-bold rounded bg-[var(--green-soft)] text-[var(--green)]">无 OS 阻碍</span>
            )}
          </div>
          <div className="space-y-1 text-[11px]">
            {os.items.slice(0, 3).map((item, i) => (
              <div key={i} className="p-1 rounded bg-[var(--red-soft)] text-[var(--red)] font-medium truncate">
                • {item.raw_text}
              </div>
            ))}
            {os.items.length === 0 && (
        ...
        <div className="p-3.5 rounded-2xl border space-y-1.5" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}>
          <div className="flex items-center justify-between text-[11px] font-bold">
            ...
            <span className={`px-1.5 py-0.5 text-xs font-bold rounded ${risk.length > 0 ? 'bg-[var(--yellow-soft)] text-[var(--yellow)]' : 'bg-[var(--green-soft)] text-[var(--green)]'}`}>
          ...
          <div className="space-y-1 text-[11px]">
            {risk.length > 0 ? (
              risk.map((r, i) => (
                <p key={i} className="text-[var(--yellow)] font-medium">⚠️ {r}</p>
              ))
            ) : (
              <p className="text-[var(--green)] font-medium">✅ 材料与推进节点正常</p>
            )}
          </div>
        </div>
        ...
        <CaseFolderCard
          ...
          folderPath={context.folder_path ?? currentCase?.folderPath}
          folderMode={context.folder_mode ?? currentCase?.folderMode}
        />
      </div>
    </div>
  );
