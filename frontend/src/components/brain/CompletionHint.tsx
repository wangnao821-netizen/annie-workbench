...
export function CompletionHint({ missingCategories }: CompletionHintProps) {
  ...
  return (
    ...
    >
      ...
      {expanded && (
        <div className="pt-1.5 border-t text-[11px] text-muted leading-relaxed space-y-1" style={{ borderColor: 'var(--border)' }}>
          <p>当前案件关键四要素 (收入、签证/身份、就业、负债) 尚未完全收齐。</p>
          <p className="text-[11px] opacity-80">补充后 AI 将自动提取并更新客户全景事实卡。</p>
        </div>
      )}
    </div>
  );
