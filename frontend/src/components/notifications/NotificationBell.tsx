...
export function NotificationBell() {
  ...
  const renderIcon = (level: NotificationLevel) => {
    switch (level) {
      case 'urgent':
        return (
          <div className="w-6 h-6 rounded-lg bg-[var(--red-soft)] border border-[var(--red-soft)] flex items-center justify-center flex-shrink-0">
            <AlertTriangle className="w-3.5 h-3.5 text-[var(--red)]" />
          </div>
        );
      case 'success':
        return (
          <div className="w-6 h-6 rounded-lg bg-[var(--green-soft)] border border-[var(--green-soft)] flex items-center justify-center flex-shrink-0">
  ...
  return (
    <div className="relative flex items-center" ref={panelRef}>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span
            className="absolute -top-1 -right-1 px-1 py-0.2 text-[11px] font-bold text-white rounded-full flex items-center justify-center min-w-[16px] h-[16px] border-2 shadow-xs"
            style={{ backgroundColor: 'var(--red)', borderColor: 'var(--bg-app)' }}
            ...
            >
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  {unreadCount > 0 && (
                    <span className="px-1.5 py-0.5 rounded-md text-xs font-bold bg-[var(--accent-soft)] text-[var(--accent)]">
                      {unreadCount} 未读
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {notifications.length > 0 && (
                    <motion.button
                      whileTap={{ scale: 0.95 }}
                      onClick={clearAll}
                      className="text-[11px] font-medium text-muted hover:text-[var(--red)] flex items-center space-x-1 cursor-pointer"
