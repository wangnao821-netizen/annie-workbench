...
interface TaskState {
  ...
  dispatchTaskAction: (taskId: number, action: 'approve' | 'reject' | 'defer' | 'delegate' | 'claim') => Promise<void>;
      ...
      if (action === 'defer') {
        ...
        set((state) => ({
          tasks: state.tasks.map((t) =>
            t.id === taskId
              ? { ...t, status: 'in_progress', assignee: 'vera', delegatedTo: 'vera', completed: false }
              : t
          ),
        }));
        showToast('success', '已认领跟进');
      } else {
        set((state) => ({
