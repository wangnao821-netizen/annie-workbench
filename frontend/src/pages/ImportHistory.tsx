...
export function ImportHistory() {
  ...
  const renderSourceBadge = (source: string) => {
    switch (source.toLowerCase()) {
      case 'vba':
        return <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-[var(--accent-soft)] text-[var(--accent)] border border-[var(--accent-soft)]">VBA 数据库</span>;
      case 'libratom':
        return <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-[var(--purple-soft)] text-[var(--purple)] border border-[var(--purple-soft)]">Libratom PST</span>;
      case 'manual':
        return <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-[var(--green-soft)] text-[var(--green)] border border-[var(--green-soft)]">手动导入</span>;
      default:
        return <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-[var(--yellow-soft)] text-[var(--yellow)] border border-[var(--yellow-soft)]">{source}</span>;
    }
  };
  ...
  const renderStatusBadge = (status: string) => {
    if (status === 'done') {
      return (
        <span className="flex items-center space-x-1 text-[var(--green)] text-xs font-bold">
    ...
    if (status === 'failed') {
      return (
        <span className="flex items-center space-x-1 text-[var(--red)] text-xs font-bold">
    ...
    return (
      <span className="flex items-center space-x-1 text-[var(--yellow)] text-xs font-bold">
    ...
    >
      ...
      {loading ? (
        ...
        <div className="rounded-2xl p-12 border flex flex-col items-center justify-center text-center space-y-2.5" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}>
          <div className="w-12 h-12 rounded-full flex items-center justify-center bg-[var(--green-soft)] text-[var(--green)]">
